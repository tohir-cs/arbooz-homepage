...
function ProductCard({ product }: { product: Product }) {
  return (
    ...
    >
      ...
      <div className="mt-5">
        <Eyebrow className="text-mocha">{product.flavor}</Eyebrow>
        <h3 className="mt-2 font-display text-heading-lg text-espresso">{product.name}</h3>
        <p className="mt-2 max-w-xs text-body-sm text-mocha">{product.description}</p>
        <div className="mt-4 flex items-baseline gap-2">
          <span className="font-sans text-body-md font-medium text-espresso">{product.price}</span>
          <span className="text-mono-xs uppercase text-ash">/ {product.unit}</span>
        </div>
      </div>
    </motion.article>
  );
        ...
        <Reveal>
          <div className="grid grid-cols-12 items-end gap-y-6 lg:gap-x-8">
            <div className="col-span-12 lg:col-span-7">
              <Eyebrow>The counter · today</Eyebrow>
              <h2
                id="todays-heading"
                className="mt-5 font-display text-display-lg text-espresso"
              >
                What's <span className="italic">fresh</span> on
                <br />
                Dzirnavu this morning.
              </h2>
            </div>
